...
* Bei Windows-VMs muss sichergestellt sein, dass die VirtIO-ISO-Datei eingebunden ist; andernfalls schlägt die Vorbereitung fehl. Der VirtIO-ISO-Treiber kann von link:https://fedorapeople.org/groups/virt/virtio-win/direct-downloads/archive-virtio/virtio-win-0.1.271-1/virtio-win-0.1.271.iso["hier,"] heruntergeladen werden. Das Skript erkennt das eingebundene Laufwerk und kopiert die benötigten Dateien automatisch.
+
** Stellen Sie sicher, dass die im Link angegebene ISO-Datei verwendet wird, da das Vorbereitungsskript das .msi-Paket zur Installation der Treiber und qemu-guest-agents verwendet. Sobald die Voraussetzungen erfüllt sind, melden Sie sich an der Shift toolkit UI an und konfigurieren Sie den Standort mit Proxmox VE als Ziel-Hypervisor. Um hinzuzufügen, klicken Sie auf „Add New Site“ und wählen Sie „Destination“ aus.
...
image::shift-toolkit-469.png[VM-Festplattenabbild in den Proxmox VE NFS-Speicherpool hochgeladen]
