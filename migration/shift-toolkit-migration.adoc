...
* link:shift-toolkit-migrate-esxi2osvmtv.html["VMware ESXi zu Red Hat OpenShift Virtualization mithilfe des Migration Toolkit for Virtualization (MTV)"]
* link:shift-toolkit-migrate-esxi2proxmox.html["VMware ESXi zu Proxmox VE"]
...
[[delete-a-blueprint]]
== Einen Bauplan löschen

Ein Blueprint, der noch nie ausgeführt wurde, kann direkt über die Benutzeroberfläche gelöscht werden. Nachdem ein Blueprint jedoch ausgelöst wurde, werden Auftragsverlaufsdatensätze erstellt, die entfernt werden müssen, bevor der Blueprint selbst gelöscht werden kann. Das PowerShell Entfernungsskript dient dazu, diese Auftragsdatensätze zu bereinigen. Danach ist das Löschen des Blueprints und der zugehörigen Ressourcengruppe möglich.

.Schritte
. Das Entfernungsskript kann über die Shift Toolkit UI unter *Einstellungen > Entwicklerzugriff > Skriptblock* heruntergeladen werden.
+
image::shift-toolkit-082.png[Jobs basierend auf dem Blueprint-Namen entfernen]

. Den ZIP-Ordner extrahieren. Das Paket enthält zwei Dateien:
+
** `removeBpJobs.json` - Konfigurationsdatei
** `removeBpJobs.ps1` - PowerShell-Skript


. Bearbeiten Sie die  `removeBpJobs.json` Datei, um Folgendes anzugeben:
+
** Ausführungsname für diesen Vorgang
** Shift Toolkit IP-Adresse
** Administrator-Zugangsdaten
** Name der Blaupause zur Entfernung
+
...
. Öffnen Sie PowerShell und navigieren Sie zu dem Ordner, der die extrahierten Dateien enthält.
...
Nach erfolgreichem Abschluss des Skripts können die Blaupause und die zugehörige Ressourcengruppe über die Shift Toolkit Benutzeroberfläche gelöscht werden.
