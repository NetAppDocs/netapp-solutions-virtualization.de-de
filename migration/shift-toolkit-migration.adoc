...
Die VMs müssen während des geplanten Wartungsfensters ordnungsgemäß heruntergefahren werden.  Bevor Sie mit der Migration beginnen, vergewissern Sie sich, dass alle Voraussetzungen erfüllt sind:

* Stellen Sie sicher, dass die Shift Toolkit VM Teil der Domäne ist.
* Stellen Sie sicher, dass die CIFS-Freigabe mit den entsprechenden Berechtigungen konfiguriert ist
* Überprüfen Sie, ob der für die Migration oder Konvertierung verwendete Qtree den korrekten Sicherheitsstil aufweist.
* Testen Sie die VM-Erstellung mit dem Hyper-V-Manager von einem beliebigen Hyper-V-Host innerhalb des Clusters und platzieren Sie die VHDx-Datei auf der CIFS-Freigabe.
* Testen Sie die VM-Erstellung aus der Shift Toolkit VM durch Hinzufügen von Hyper-V-Verwaltungstools über „Programme und Funktionen“ oder PowerShell: `add-windowsfeature rsat-hyper-v-tools`
