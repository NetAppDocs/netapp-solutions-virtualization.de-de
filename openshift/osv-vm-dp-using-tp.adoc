...
Die persistenten Datenträger der OpenShift Virtualisierungs-VMs können durch ONTAP-Speicher, der in den OpenShift Cluster integriert ist, mit link:https://docs.netapp.com/us-en/trident/["Trident CSI"] gesichert werden. In diesem Abschnitt verwenden wir link:https://docs.netapp.com/us-en/trident/trident-protect/learn-about-trident-protect.html["Trident Protect"] , um Snapshots und Backups von VMs einschließlich ihrer Datenvolumes im ONTAP Object Storage zu erstellen.

Bei Bedarf stellen wir dann eine Wiederherstellung aus einem Snapshot oder einem Backup her.

Trident Protect ermöglicht Snapshots, Backups, Wiederherstellung und Notfallwiederherstellung von Anwendungen und VMs auf einem OpenShift Cluster. Für OpenShift Virtualization VMs können mit Trident Protect Daten geschützt werden, darunter Kubernetes-Ressourcenobjekte, persistente Volumes und interne Images.
...
[%collapsible]
====
Vor dem Erstellen von Snapshots und Backups für eine Anwendung oder eine VM muss ein Objektspeicher in Trident Protect konfiguriert werden, um die Snapshots und Backups zu speichern. Dies erfolgt mithilfe des Bucket-CR. Nur Administratoren können einen Bucket-CR erstellen und konfigurieren. Der Bucket-CR wird in Trident Protect als AppVault bezeichnet. AppVault-Objekte sind die deklarative Kubernetes-Workflow-Darstellung eines Speicher-Buckets. Ein AppVault-CR enthält die Konfigurationen, die erforderlich sind, damit ein Bucket in Schutzvorgängen wie Backups, Snapshots, Wiederherstellungsvorgängen und SnapMirror-Replikation verwendet werden kann.
...
== Wiederherstellung einer einzelnen Datei für eine VM

Diese Methode zur Wiederherstellung einzelner Dateien ist nur für NAS-/dateibasierte Volumes anwendbar, die als Dateisystem im Client-Pod eingebunden sind, wie z. B. NFS-Volumes. Sie gilt nicht für SAN-/Raw-Block-Volumes, einschließlich VM-Festplatten, die mit  `volumeMode: Block` wie zuvor beschrieben bereitgestellt wurden.

Bei unterstützten NAS-/dateibasierten Volumes lässt sich eine bestimmte Datei einfach wiederherstellen, indem das Snapshot-Verzeichnis auf dem zugrunde liegenden Volume für den Client sichtbar gemacht wird. Dies kann durch Bearbeiten der Trident Backend-Konfiguration erfolgen, indem der Abschnitt defaults hinzugefügt und der  `snapshotDir` Parameter auf  `true` gesetzt wird. Dadurch wird der Zugriff auf das  `.snapshot` Verzeichnis vom selben Mountpunkt wie das Hauptvolume ermöglicht. Das  `.snapshot` Verzeichnis enthält alle Snapshots. Es besteht die Möglichkeit, in das gewünschte Snapshot-Verzeichnis zu wechseln, die wiederherzustellende Datei zu finden und sie an den gewünschten Speicherort zu kopieren.


NOTE: Diese Methode legt historische Daten für Workloads/Benutzer mit Mount-Zugriff offen und kann gegen das Prinzip der minimalen Rechtevergabe/Compliance verstoßen. Es sollte der Einfluss auf Sicherheit und Betrieb berücksichtigt werden, zudem ist zu beachten, dass diese Option nur temporär aktiviert und nach der Wiederherstellung einer Datei wieder deaktiviert werden sollte. Wenn NFS Version 4 verwendet wird, ist das  `.snapshot` Verzeichnis beim Auflisten von Dateien mit einem  `ls` Befehl nicht sichtbar. Es ist jedoch möglich, mit  `cd` auf das  `.snapshot` Verzeichnis zuzugreifen.

.Beispiel für die Wiederherstellung einzelner Dateien durch Zugriff auf das .snapshot-Verzeichnis und Kopieren einer Datei aus einem Snapshot
[%collapsible]
====
[source, yaml]
----
...
----
image:rh-os-n-use-case-ocpv-tp-dp-sfr-001.png["Pod und PVC der VM"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-002.png["ONTAP Volume Name für die VM-Festplatte"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-003.png["Snapshot-Verzeichnis im Client sichtbar"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-004.png["Mount-Verzeichnis in der VM"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-005.png["Verlust einer Datei in der VM und Kopieren der Datei aus dem Snapshot-Verzeichnis in das Hauptverzeichnis zur Wiederherstellung"]
