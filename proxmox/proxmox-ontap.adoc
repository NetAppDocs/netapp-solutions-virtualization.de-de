...
* Integration von NetApp Console und Data Infrastructure Insights .
...
. Um die nConnect-Option zu aktivieren, öffnen Sie die Shell auf einem beliebigen Knoten des Clusters und geben Sie pvesm set pvenfs01 --options nconnect=4 ein, wobei pvenfs01 die im vorherigen Schritt erstellte Speicher-ID ist. Beachten Sie, dass bei Verwendung der Trunking-Funktion mindestens NFS v4.1 verwendet und die Option „trunkdiscovery“ aktiviert sein muss. Führen Sie dazu folgenden Befehl aus: `pvesm set pvenfs01 --options vers=4.1,trunkdiscovery`
