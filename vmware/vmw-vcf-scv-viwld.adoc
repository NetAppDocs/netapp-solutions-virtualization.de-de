...
TIP: Zusätzlich zur Replikation von mit SCV erstellten Backups auf Sekundärspeicher können mit NetApp Backup and Recovery for VMs auch Offsite-Kopien von Daten auf Objektspeicher bei einem der drei (3) führenden Cloud-Anbieter erstellt werden. Weitere Informationen finden Sie in diesem Angebot.link:https://docs.netapp.com/us-en/data-services-backup-recovery/index.html["NetApp Backup and Recovery"] Die
